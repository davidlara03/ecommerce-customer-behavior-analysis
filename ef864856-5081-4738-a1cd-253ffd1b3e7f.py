...
#     
# Los cambios se aplicaron correctamente, bien hecho!
# 
...
def clean_user(user_info, name_index, age_index):
    #user_name_1= user_info[name_index].strip().replace("_","").little()# define tu función aquí
    user_name_1= user_info[name_index].strip().replace("_","").lower()# define tu función aquí
...
# 
# 
# Hice una pequeña corrección en el código, reemplace `little()` por `lower()` que es el nombre correcto del método para convertir las cadenas en minusculas
# </div>
...
for categoria in fav_categories:
    fav_categories_low.append(categoria.lower())# escribe tu código aquí
...
for user in users:
    ...
    user.pop(3)
    #user.insert(3)
    user.insert(3, categories_low) # <- categories_low hacía falta
    users_categories_low.append(user)# escribe tu código aquí
...
# Bien hecho!<br/>
# 
# Has integrado de una manera casi correcta el código de `lower()`, había un pequeño error al usar `insert()`, el cual necesita dos parametros, la posición donde se va a insertar el nuevo elemento y también dicho elemento, en este caso `categories_low`
# </div>
# 
...
def clean_user(user_info, name_index, age_index, cat_index):
  ...
  user_name_1 = user_info[name_index].strip().lower().replace('-', ' ') # escribe tu código aquí
  ...
  for category in user_info[cat_index]:
        categories_low.append(category.lower())# escribe tu código aquí
...
for user in users:
    if user[2] > 30:
        print(" ".join(user[1]))# escribe tu código aquí
...
# Cuidado!<br/>
# 
# Buen uso de la condición `if`, pero estas obteniendo los valores `mayores` a `30` (`>30`), cuando el ejercicio pide `menores` a `30` (`<30`)
# </div>
...
# Correcto!<br/>
# 
# Buena manera de implementar ambas condiciones `if`
# </div>
# 
...
for user in users:
    #for cateogry in user[3]:
    if 'clothes' in user[3]:
        print(f"{' '.join(user[1])}, {user[2]}")# escribe tu código aquí
...
# Cuidado!<br/>
# 
# Buen uso de `in` para buscar la cadena dentro de la lista, sin embargo se estaba usando `for` en lugar de `if` para la condición
# </div>
# 
...
# Cuidado!<br/>
# 
# No se esta mandando a llamar a la función de manera adecuada, necesitas completar este paso, un ejemplo de como lo puedes hacer es de la siguiente manera:
#     
# ```
# result=get_client_by_cat(users,0,1,2,3,4,'home')
# ```
# 
# De esta manera estas mandando la lista completa de usuarios y los indices necesarios para cada una de las variables
# </div>
# 
